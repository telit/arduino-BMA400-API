...
static void test_bma400_orient_feature_int(struct bma400_orient_int_conf test_orient_conf, struct bma400_dev *dev)
{
    ...
    accel_settin[0].param.orient.int_chan = BMA400_UNMAP_INT_PIN;
    ...
    Serial.println("Shake board for Orientation change interrupt");
    ...
    while (1)
    {
        ...
        {
            Serial.println("Orientation interrupt detected");

            if (orient_cnter == 0)
            {
              Serial.print("Reference :  X : ");
              Serial.print(accel_settin[0].param.orient.orient_ref_x);
              Serial.print("    Y : ");
              Serial.print(accel_settin[0].param.orient.orient_ref_y);
              Serial.print("    Z : ");
              Serial.println(accel_settin[0].param.orient.orient_ref_z);
            }
        ...
        {
            Serial.println("Orientation interrupt test done. Exiting!");
            break;
        }
        delay(100);
    }
...
void setup() {
  ...
  Serial.println("Orient change interrupt with XYZ Axes enabled");
  ...
  Serial.println("Orient change interrupt with Y Axis enabled");
  ...
  Serial.println("Orient change interrupt with X Axis enabled");
  ...
  Serial.println("Orient change interrupt with Z Axis enabled");
  ...
  Serial.println("Orient change interrupt with Z Axis enabled and manual update");
  test_orient_conf.axes_sel = BMA400_AXIS_Z_EN;
  test_orient_conf.data_src = BMA400_DATA_SRC_ACC_FILT2;
  test_orient_conf.int_chan = BMA400_INT_CHANNEL_1;
  test_orient_conf.orient_int_dur = 7; /* 10ms/LSB */
